...
do
  ...
  vim.keymap.set('n', '<leader>p', vim.cmd.Ex, { desc = 'Open file ex[p]lorer' })
...
do
  ...
  gitsigns.setup {
    ...
    -- gitsigns.nvim's recommended keymaps (enabled):
  ...
  -- NOTE: an explicit `name` is given because the repository is called `nvim`,
  --  which would otherwise be the plugin's directory name on disk.
  vim.pack.add { { src = gh 'catppuccin/nvim', name = 'catppuccin' } }
  require('catppuccin').setup {}

  -- Load the colorscheme here.
  -- Catppuccin ships four flavours: 'catppuccin-latte', 'catppuccin-frappe',
...
do
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      end, { buffer = buf, desc = '[G]oto Definition in [V]ertical Split' })

      vim.keymap.set('n', 'gh', function()
        vim.cmd.wincmd 's'
        builtin.lsp_definitions()
      end, { buffer = buf, desc = '[G]oto Definition in [H]orizontal Split' })
...
do
  ...
  require('mason-lspconfig').setup {
    -- Automatically `vim.lsp.enable()` every server that Mason has installed.
    -- This is what keeps the servers in `ensure_installed` below attaching without
    -- having to also list each one in the `servers` table above.
...
do
  ...
  require('conform').setup {
    ...
    format_on_save = function(bufnr)
      -- Format on save for everything *except* the languages listed here, which
